...
# User credentials
USERNAME = "jhananishri214@gmail.com"
PASSWORD = "Jhananishri-111"
...
# Step 2: Query the Catalog using OData API
def search_products(access_token, lat, lon, start_date, end_date, product_type):
    ...
    url = f"https://catalogue.dataspace.copernicus.eu/odata/v1/Products?$filter={filter_query}&$top=10&$orderby=ContentDate/Start asc"
...
def process_csv_and_download_data(input_csv, output_csv):
    ...
    SAVE_DIR = "satellitedata"  # Specify your desired directory
    ...
    for index, row in df.iterrows():
        ...
        if "value" in search_result and search_result["value"]:
            product_id = search_result["value"][0]["Id"]  # Get the first product found
...
output_csv = 'merged_output_with_paths.csv'
